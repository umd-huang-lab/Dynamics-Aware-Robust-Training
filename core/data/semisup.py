...
def get_semisup_dataloaders(train_dataset, test_dataset, val_dataset=None, batch_size=256, batch_size_test=256, num_workers=4, 
    ...
    """
    dataset_size = train_dataset.dataset_size # 50000 for cifar10 (not counting aux data, not discounting validation set)
#     print('get_semisup_dataloaders: dataset_size:{} num_batches:{}'.format(dataset_size,int(np.ceil(dataset_size/batch_size))))
...
class SemiSupervisedDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, base_dataset='cifar10', take_amount=None, take_amount_seed=13, aux_data_filename=None, 
                 add_aux_labels=False, aux_take_amount=None, aux_ind_pth = None, train=False, validation=False, 
                 **kwargs):
        
        '''
        aux_ind_use: only use the aux data indexed by aux_ind in aux_ind_pth
        
        '''
        if aux_take_amount is not None:
            assert aux_ind_pth is None, 'aux_take_amount and aux_ind_pth cannot be both specified currently'
        ...
        if self.train:
            ...
            if aux_data_filename is None:
                print('SemiSupervisedDataset: Not using additional aux data')
            ...
            if aux_data_filename is not None:
                ...
                if os.path.splitext(aux_path)[1] == '.pickle':
                    ...
                    raise ValueError('SemiSupervisedDataset: Carmon dataset cannot be used currently')
                else:
                    ...
                    if aux_ind_pth is not None:
                        aux_ind = np.load(aux_ind_pth)
                        aux_data = aux_data[aux_ind]
                        aux_targets = aux_targets[aux_ind]
                        print('Using {:.2f}% extra data (total number {}) from {}'.format(aux_ind.mean() * 100, \
                                              aux_data.shape[0],aux_path))
                    else:
                        print('Using all extra data (total number {}) from {}' .format(aux_data.shape[0], aux_path))
...
class SemiSupervisedSampler(torch.utils.data.Sampler):
    """
    Balanced sampling from the labeled and unlabeled data, for training data (not val or test data)
    """
    def __init__(self, sup_inds, unsup_inds, batch_size, unsup_fraction=0.5, num_batches=None):
        ...
        if len(unsup_inds) == 0:
            ...
            unsup_fraction = 0
            print('Training sampler: Not using aux data')
        
        else:
            if unsup_fraction is None or unsup_fraction < 0:
                # if not specify unsup_fraction, sample according size of train and aux_train
        ...
        print('Training sampler: sup_batch_size is ',self.sup_batch_size)
    ...
    def __iter__(self):
        ...
        while batch_counter < self.num_batches:
            # it seems that here in one epoch, some examples will not be visited
            # this can help fixing the total number of iterations = num_epochs * num_batches 
            # so 100 epochs with or without aux data, go through the same amount of iterations/example_count

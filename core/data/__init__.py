...
def load_data(data_dir, batch_size=256, batch_size_test=256, num_workers=4, use_augmentation=False, shuffle_train=True, 
              aux_data_filename=None, aux_ind_pth = None, unsup_fraction=None, validation=False):
    """
    Returns train, test datasets and dataloaders.
    Arguments:
        aux_ind_pth(str): path to the index of aux data that will be used for training
    ...
    if validation:
        if dataset in SEMISUP_DATASETS:
            train_dataset, test_dataset, val_dataset = load_dataset_fn(data_dir=data_dir, use_augmentation=use_augmentation, 
                                    aux_data_filename=aux_data_filename, aux_ind_pth=aux_ind_pth, validation=True)
        elif dataset == 'tiny-imagenet':
            train_dataset, test_dataset, val_dataset = load_dataset_fn(data_dir=data_dir, use_augmentation=use_augmentation, 
                                                        validation=True)
        else:
            raise ValueError('Validation set not available for dataset :{}'.format(dataset))
  
    else:
        if dataset == 'cifar100s':
            train_dataset, test_dataset, _ = load_dataset_fn(data_dir=data_dir, use_augmentation=use_augmentation)
        else:
            train_dataset, test_dataset = load_dataset_fn(data_dir=data_dir, use_augmentation=use_augmentation)
        print('\nNot using validation set and aux data\n')
    ...
    if dataset in SEMISUP_DATASETS:
        ...
        test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size_test, shuffle=False, 
                                                      num_workers=num_workers, pin_memory=pin_memory)
        if validation and dataset == 'tiny-imagenet':

...
from core.DyART.DyART_Linf import DyART_loss_Linf
...
class WATrainer(Trainer):
    ...
    def __init__(self, info, args):
        ...
        self.eval_attack = create_attack(self.wa_model, CWLoss, args.attack, args.attack_eps, 4*args.attack_iter, 
                                         args.attack_step) 
        print('{}: Using CW loss for attack when testing using attack method:{}'.format(self.norm_type,args.attack))
    ...
    def train(self, dataloader, epoch=0, adversarial=False, verbose=True):
        ...
        for data in tqdm(dataloader, desc='Epoch {}: '.format(epoch), disable=not verbose):
            ...
            if adversarial:
                if self.params.DyART:
                    if self.norm_type == 'Linf':
                        loss, batch_metrics = self.DyART_loss_Linf(x,y)
                    else:
                        raise ValueError('DyART only supports Linf currently')
#                 elif self.params.beta is not None and self.params.mart:
                elif self.params.beta > 0 and self.params.mart:
...
#                 elif self.params.beta is not None:
    ...
    def DyART_loss_Linf(self,x,y):
        loss, batch_metrics = DyART_loss_Linf(model=self.model,X=x,y=y,lam_robust=self.params.lam_robust,\
              temperature=self.params.temperature,\
              h_prime_alpha=self.params.h_prime_alpha,\
              h_prime_r0=self.params.h_prime_r0,\
              iter_FAB=self.params.iter_FAB,eps_FAB=self.params.eps_FAB, use_high_quality=self.params.use_high_quality,\
              eps_eval = 0.031)
